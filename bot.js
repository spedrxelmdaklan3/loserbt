...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = ['h!help For Help','Sub To iiLoSeR_','ask DaklinX#5283 For Help','You need activate get message form server members in you user settings to get messages from the bot'];
    var i = -1;
    var j = 0;
...
if (message.content.startsWith(prefix + 'help')) { /// This is The DMS Code Send The Help In DMS // Code By DaklinX
    let pages = [`**
        ***__General orders__***
**
You need activate get message form server members in you user settings to get messages from the bot
...
${prefix}bordcast.your.text / رسالة جماعية الى كل اعضاء السيرفر
 ...
 client.on('message', message => {
    ...
    message.channel.send({
        embed: new Discord.RichEmbed()
            ...
            .setColor('RANDOM')
            .setTitle('``INFO DaklinX Community ©`` ')
...
client.on('message', message => {
    ...
    translate(finalToTrans, {to: language}).then(res => {
            message.channel.send({embed: {
                color: 3447003,
                author: {
                  name: 'DaklinX Community ©\'s translator',
...
message.react("?")
 ...
 client.on('message', message => {
  if(!message.channel.guild) return;
if(message.content.startsWith('h!bordc')) {
if(!message.channel.guild) return message.channel.send('**هذا الأمر فقط للسيرفرات**').then(m => m.delete(5000));
if(!message.member.hasPermission('ADMINISTRATOR')) return      message.channel.send('**للأسف لا تمتلك صلاحية** `ADMINISTRATOR`' );
let args = message.content.split(" ").join(" ").slice(2 + prefix.length);
let copy = "DaklinX Community ©";
...
client.on('guildCreate', guild => {
  ...
  .setDescription(`**
  New Server Add DaklinX Community © ?
...
client.on('guildDelete', guild => {
     ...
     .setURL('https://discordapp.com/oauth2/authorize?client_id=435392018693488641&scope=bot&permissions=2146958591')
  .setDescription(`**
  Server Kicked DaklinX Community © :cry:
اسم السيرفر: ${guild.name}
صاحب السيرفر: ${guild.owner}**`);
client.channels.get("474127377245667328").sendEmbed(embed)
});
